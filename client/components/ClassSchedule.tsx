...
import { Calendar, Clock, MapPin, Users, Plus, MoreHorizontal, ChevronRight } from 'lucide-react';
...
export const ClassSchedule: React.FC = () => {
  ...
  const filteredClasses = MOCK_CLASSES
      ...
      <div className="space-y-4">
        {filteredClasses.length > 0 ? (
          filteredClasses.map((session) => (
            ...
            >
              {/* Color Stripe */}
              <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${getColorClasses(session.color)}`}></div>
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-6 pl-2">
                ...
                <div className="flex-shrink-0 min-w-[140px]">
                  <div className="flex items-center gap-2 text-slate-900 font-bold text-lg">
                    ...
                    {session.startTime} <span className="text-slate-400 font-normal text-sm">- {getEndTime(session.startTime, session.duration)}</span>
                  </div>
                ...
                <div className="flex-grow">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${getLevelBadgeColor(session.level)}`}>
                      {session.level}
                    </span>
                  </div>
                ...
                <div className="flex-shrink-0 flex items-center gap-6 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-slate-100">
                  <div className="flex flex-col gap-1 w-full md:w-32">
                    <div className="flex justify-between text-xs font-medium">
                      ...
                      <span className={session.students >= session.capacity ? 'text-red-500' : 'text-slate-900'}>
                        {session.students}/{session.capacity}
                      </span>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full ${session.students >= session.capacity ? 'bg-red-500' : 'bg-indigo-500'}`} 
          ...
          <div className="text-center py-16 bg-white rounded-xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 mt-1 mb-6">There are no classes scheduled for {selectedDay}.</p>
