...
/**
 * Record / edit attendance for a single class session.
 * Shared by the instructor and manager views — the backend authorises the
 * acting user (own class for instructors, own studio for admins) and always
 * keeps the class's real instructor as the attendance owner.
 */
export const AttendanceModal: React.FC<AttendanceModalProps> = ({
  ...
  const handleSave = async (sessionStatus: "IN_PROGRESS" | "COMPLETED") => {
    ...
    // A session can only be *completed* once every student has been marked;
    // otherwise it can only be saved as a draft.
    if (sessionStatus === "COMPLETED" && !allMarked) {
      toast.error("יש לסמן נוכחות לכל התלמידים לפני סיום הדיווח");
      return;
    }
    ...
    try {
      ...
      toast.success(
        sessionStatus === "COMPLETED" ? "נוכחות נשמרה בהצלחה" : "טיוטה נשמרה"
      );
      if (onSaved) onSaved();
      onClose();
    } catch (error) {
      ...
      <div className="flex flex-col h-[600px]">
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-between gap-3">
          ...
          <button
            ...
            disabled={saving || records.length === 0 || !allMarked}
            title={
              !allMarked ? "יש לסמן נוכחות לכל התלמידים כדי לסיים" : undefined
            }
            className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Check size={18} />
            סיום דיווח
          </button>
