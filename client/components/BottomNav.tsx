...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab, onLogout, userRole }) => {
  ...
  const allMenuItems = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'ראשי', roles: ['ADMIN', 'INSTRUCTOR', 'STUDENT', 'SUPER_ADMIN'] },
    { id: 'browse', icon: Search, label: 'קורסים', roles: ['STUDENT'] },
    { id: 'schedule', icon: Calendar, label: 'מערכת', roles: ['ADMIN', 'INSTRUCTOR'] },
    { id: 'students', icon: Users, label: 'תלמידים', roles: ['ADMIN', 'INSTRUCTOR'] },
    { id: 'payments', icon: CreditCard, label: 'תשלומים', roles: ['ADMIN'] },
    { id: 'administration', icon: Users, label: 'ניהול', roles: ['ADMIN', 'SUPER_ADMIN'] },
    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-slate-200 shadow-[0_-2px_10px_rgba(0,0,0,0.08)] dark:bg-slate-900 dark:border-slate-700">
      <div className="flex items-center justify-around px-1 py-1">
        ...
        >
          ...
          <span className="text-[10px] mt-0.5 font-medium">{isDarkMode ? 'בהיר' : 'כהה'}</span>
        </button>
