...
export const EnrollStudentModal: React.FC<EnrollStudentModalProps> = ({
  ...
  const handleEnroll = async (classId: string) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'שגיאה ברישום');
    } finally {
      setEnrollingId(null);
    }
  };
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title={`רישום ${studentName} לקורסים`}>
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <p className="text-center py-8 text-slate-500 dark:text-slate-400">אין קורסים פעילים כרגע.</p>
        ) : (
          courses.map(course => (
            <div
              key={course.id}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-slate-800 dark:text-slate-200">{course.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {DAY_MAP[course.day_of_week]} • {course.start_time?.substring(0, 5)} • {course.current_enrollment}/{course.max_capacity} תלמידים
                    {course.price_ils > 0 && <span className="mr-1">• ₪{course.price_ils}</span>}
                  </p>
                </div>
              </div>
              <button
                onClick={() => handleEnroll(course.id)}
                disabled={enrollingId === course.id || successId === course.id}
              ...
              >
                {enrollingId === course.id ? (
                  ...
                  <span className="flex items-center gap-1"><Check size={14} /> נרשם!</span>
                ) : (
                  'רשום'
                )}
              </button>
