...
export class UserController {
  ...
  static async validateStudio(req: Request, res: Response, next: NextFunction) {
    ...
    if (!serialNumber) {
      return res
        .status(400)
        .json({ error: "Studio serial number is required" });
    }
    ...
    try {
      ...
      if (!studio) {
        return res
          .status(404)
          .json({ valid: false, message: "Studio not found" });
      }
  ...
  static async prepareRegistration(
    ...
    requestLog.info(
      { email, serialNumber, hasToken: !!invitationToken },
    ...
    try {
      ...
      if (invitationToken) {
        ...
        if (!invitation.studioId) {
          return res
            .status(400)
            .json({ error: "Invitation token is missing a valid Studio ID" });
        }

        studioId = invitation.studioId;
        invitationRole = invitation.role as
          | "ADMIN"
          | "INSTRUCTOR"
          | "SUPER_ADMIN";
      }
      // Otherwise, validate the studio serial number
        ...
        return res
          .status(400)
          .json({
            error: "Either serialNumber or invitationToken is required",
          });
      }

      // Create pending registration with validated studio_id and role (if from invitation)
      const pendingReg = await UserService.createPendingRegistration(
        email,
        studioId,
        invitationToken,
        invitationRole
      ...
      requestLog.info(
        { email, studioId },
        "Registration prepared successfully"
      );
      res.status(200).json({
        success: true,
